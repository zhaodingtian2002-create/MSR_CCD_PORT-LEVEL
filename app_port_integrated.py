...
st.set_page_config(
    page_title="MSR Port Coordination and Pollution Risks",
    page_icon="⚓",
    layout="wide",
...
def load_indicator_table(folder):
    ...
    fallback_path = os.path.join(folder, "MSR_Port_CCD_Direct_Results.csv")
    fallback = None
    if os.path.exists(fallback_path):
        fallback = pd.read_csv(fallback_path)
        fallback.columns = fallback.columns.str.strip().str.lower()
        if "port_name" in fallback:
            fallback["port_name"] = normalize_port_name(fallback["port_name"])
            fallback = fallback[~fallback["port_name"].isin(EXCLUDED_PORTS)].copy()
            fallback = fallback.groupby("port_name", as_index=False).mean(numeric_only=True)
    ...
    for path in sorted(glob(os.path.join(folder, "MSR_Port_*.csv"))):
        ...
        recovered_values = 0
        if fallback is not None and indicator in fallback:
            fallback_values = fallback[["port_name", indicator]].rename(
                columns={indicator: "fallback_value"}
            )
            frame = frame.merge(fallback_values, on="port_name", how="outer")
            missing_before = frame[indicator].isna()
            frame[indicator] = frame[indicator].fillna(frame["fallback_value"])
            recovered_values = int((missing_before & frame[indicator].notna()).sum())
            frame = frame.drop(columns="fallback_value")
        missing_rate = float(frame[indicator].isna().mean())
        if missing_rate == 1:
            status = "Excluded: all values missing"
        elif recovered_values:
            status = f"Included: {recovered_values} values recovered from direct results"
        else:
            status = "Included"
        quality.append(
            {
...
def render_ccd_module(ccd, weights, normalized):
    st.markdown(
        '<div class="section-note">The Coupling Coordination Degree (CCD) measures the balance between the geo-environmental and socio-economic subsystems at port level. Pollution is evaluated separately so development capacity is not mixed with environmental risk.</div>',
        unsafe_allow_html=True,
    )
    ...
    with subtab2:
        ...
        fig = px.scatter(
            ...
            labels={
                "geo_score": "Geo-environmental subsystem score",
                "socio_score": "Socio-economic subsystem score",
                "ccd_type": "Development type",
            },
        ...
        st.plotly_chart(fig, use_container_width=True)
        st.caption(
            "Both subsystem scores range from 0 to 1. Dashed lines show the shared sample-median threshold used to classify the four development types."
        )
...
def render_pollution_module(pollution, pollution_wide, ccd):
    ...
    subtab1, subtab2, subtab3 = st.tabs(
        ["Pollution map", "Time series", "Port comparison"]
    )
...
def render_integrated_risk_module(combined, concentration_cut, exposure_cut):
    st.markdown(
        '<div class="section-note">This module identifies comparative environmental and exposure risks by combining development coordination, pollution concentration, and population-weighted exposure. Thresholds are relative to the observed MSR port sample and are not regulatory limits.</div>',
        unsafe_allow_html=True,
    )
    subtab1, subtab2, subtab3 = st.tabs(
    ...
    with subtab1:
        fig = px.scatter(
            ...
            labels={
                ...
                "exposure_quadrant": "Pollution-exposure risk quadrant",
            },
        ...
        render_point_map(
            ...
            quadrant_colors,
            [
                ("exposure_quadrant", "Risk quadrant"),
    ...
    with subtab2:
        ...
        type_colors = {
            "Coordinated": "#2f7d5a",
            "Environment-lagging": "#2f6f9f",
            "Economy-lagging": "#d18b26",
            "Dual-lagging": "#7b4f9d",
        }
        exposure_colors = {
            "Low": "#f3b6ad",
        ...
        parents = [""] * len(LEVEL_ORDER)
        values = [int((combined["ccd_type"] == ccd_type).sum()) for ccd_type in LEVEL_ORDER]
        colors = [type_colors[ccd_type] for ccd_type in LEVEL_ORDER]
        ids = [f"type:{ccd_type}" for ccd_type in LEVEL_ORDER]
        for ccd_type in LEVEL_ORDER:
            for exposure_level in ["Low", "Medium", "High"]:
                count = int(
                    (
                        (combined["ccd_type"] == ccd_type)
                        & (combined["exposure_level"] == exposure_level)
                    ).sum()
                )
                if count:
                    labels.append(exposure_level)
                    parents.append(f"type:{ccd_type}")
                    values.append(count)
                    colors.append(exposure_colors[exposure_level])
                    ids.append(f"{ccd_type}:{exposure_level}")
        sunburst = go.Figure(
            go.Sunburst(
                ids=ids,
                labels=labels,
                ...
                hovertemplate="%{label}: %{value} ports<extra></extra>",
            )
        )
        sunburst.update_layout(
            height=570,
            title="Composition of development and exposure risk levels",
            margin=dict(t=70, l=10, r=10, b=10),
        )
        st.plotly_chart(sunburst, use_container_width=True)
        st.caption(
            "Inner ring: four CCD development types. Outer ring: exposure risk from low (light red) to high (dark red)."
        )
    ...
    with subtab3:
        ...
        st.caption(
            "A higher score indicates greater combined risk and a stronger need for attention. Priority score = 40% pollution index + 40% exposure index + 20% coordination deficit, where coordination deficit = 1 − CCD coordination degree (D). The weights are exploratory screening assumptions, not regulatory or causal estimates."
        )
...
<b>Coupling Coordination Degree (CCD) formula</b><br>
Each indicator is direction-adjusted and min-max normalized. Entropy weights are calculated separately for the geo-environmental subsystem score (F) and socio-economic subsystem score (G). The model then calculates
...
""",
        ...
        excluded = quality[quality["status"].str.startswith("Excluded")]
...
<div class="app-header">
    <h1>Maritime Silk Road Port Coordination and Pollution Exposure Risks</h1>
    <p>Port-level Coupling Coordination Degree (CCD), multi-pollutant trends, population-weighted exposure, and integrated risk classification</p>
...
""",
    ...
    with st.sidebar:
        st.caption("Maritime Silk Road port-level model by ZHAO Dingitan | Student ID: 3035945807")
    ...
    tab1, tab2, tab3 = st.tabs(
        [
            ...
            "3. Environmental and exposure risks",
        ]
    )
    with tab1:
    ...
    with tab3:
        render_integrated_risk_module(combined, concentration_cut, exposure_cut)
    ...
    download3.download_button(
        ...
        "MSR_port_integrated_risk_classification.csv",
        "text/csv",
        use_container_width=True,
    )
